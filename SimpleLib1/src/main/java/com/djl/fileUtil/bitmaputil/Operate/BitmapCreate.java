 ...
 */
package com.djl.fileUtil.bitmaputil.Operate;
...
import com.djl.fileUtil.FileUtils;
...
public class BitmapCreate {
	 ...
	 *            目标高度
	 */
	public static Bitmap bitmapFromByteArray(byte[] data, int offset, int length, int reqWidth,
			int reqHeight) {
		if (reqHeight == 0 || reqWidth == 0) {
	 ...
	 *            目标高度
	 */
	public static Bitmap bitmapFromStream(InputStream is, Rect outPadding, int reqWidth,
			int reqHeight) {
		if (reqHeight == 0 || reqWidth == 0) {
			return BitmapFactory.decodeStream(is);
		} else {
